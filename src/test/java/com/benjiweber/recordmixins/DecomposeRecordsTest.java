...
import static com.benjiweber.recordmixins.DeconstructRecordsTest.If.withFallback;
import static org.junit.Assert.assertEquals;

public class DeconstructRecordsTest {
    ...
    public void example() {
        ...
        String resultMatch = withFallback("none").If.<String,String>instance(name, (first, last) ->
                first.toLowerCase() + " " + last.toLowerCase()
        );
        assertEquals("benji weber", resultMatch);
        String resultNoMatch = withFallback("none").If.<String,String>instance(nums, (first, last) ->
                first.toLowerCase() + " " + last.toLowerCase()
        );
        assertEquals("none", resultNoMatch);
    }

    interface ParamTypeAware extends MethodFinder {
        default Class<?> paramType(int n) {
            return method().getParameters()[n].getType();
        }
    }
        ...
        static <TResult> Match<TResult> withFallback(TResult defaultResult) {
            return new Match<>() {
                public <L, R> TResult instance(Object toMatch, MethodAwareBiFunction<L, R, TResult> action) {
                    return DeconstructRecordsTest.If.instance(toMatch, action).orElse(defaultResult);
                }
